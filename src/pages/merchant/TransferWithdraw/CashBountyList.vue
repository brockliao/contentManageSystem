<template>
  <div class="content">
    ...
    <div class="CashBountyListContainer">
      <div class="partTitle">
      ...
      <div class="searchContent">
        <el-form ref="searchForm" :model="searchForm" label-width="120px">
          <el-form-item label="条件筛选：">
            <el-select v-model="searchForm.status" size="small" placeholder="审核状态" class="searchContentInput">
      ...
      <div class="partTitle">
        <i class="iconfont icon-menu"></i>提现审核列表
      </div>
      <el-table :data="tableOptions.lists" border style="width: 100%">
        ...
        <el-table-column prop="_actualAmount" label="待转账金额" width="120"></el-table-column>
        <el-table-column prop="_createTime" label="申请提现时间"></el-table-column>
        <el-table-column prop="statusDisplay" label="审核状态" width="120"></el-table-column>
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button @click="descritionJump(scope.row)" :disabled="scope.row.status == 0" type="text" size="small"
              class="textButton">审核</el-button>
...
import {
  ...
  GetWithdrawCheckListToExcel
} from "./../../../Apis";
export default {
  data() {
    return {
      breadList: [{ label: "提现审核" }],
      searchForm: {
        sn: "",
        merchantId: "",
        merchantName: "",
  ...
  methods: {
    ...
    async exportData() {
      ...
      let res = await GetWithdrawCheckListToExcel(params);
      if (res && res.success) {
        downFile(res.result);
      }
    },
    ...
    descritionJump(rowData) {
      setStorage("CashBountyListRow", JSON.stringify(rowData));
    ...
    initTimes() {
      ...
      this.searchForm.beginTime = nowYear + "-" + nowMonth + "-01";
    ...
    async initLists() {
      ...
      let res = await GetWithdrawCheckList(params);
      if (res.success) {
        ...
        lists.forEach(item => {
          ...
          item["_createTime"] = dateFrom(item.createTime);
